...
if DO_RESTART:
    ...
    time.sleep(1.5)
    code, out, err = run(
        f"cd {REMOTE} && setsid nohup bash -c 'DISPLAY=:99 ACCOUNT_POOL_MIN=50 ACCOUNT_POOL_MAX=50 node server.js > server.log 2>&1' > /dev/null 2>&1 & sleep 4 && echo STARTED && tail -3 server.log")
