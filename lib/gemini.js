...
function geminiRequestToChat(body, { model, stream } = {}) {
  ...
  for (const k of ['conversation_id', 'chat_id', 'thread_id', 'character_id', 'rp_preset', 'rp_nsfw', 'web_search', 'thinking']) {
    ...
    write(chunk) {
      ...
      while ((idx = buf.indexOf('\n\n')) !== -1) {
        ...
        for (const g of openAiChunkToGemini(parsed, model)) {
          ...
          if (g.candidates && g.candidates[0] && g.candidates[0].finishReason) sentStop = true;
          emit(g);
        }
      }
      return true;
      ...
      if (mode === 'stream') {
        if (!sentStop && statusCode === 200) {
          emit({ candidates: [{ content: { parts: [], role: 'model' }, finishReason: 'STOP', index: 0 }], modelVersion: model });
        }
