...
class AccountPool {
  ...
  load() {
    ...
    try {
      if (fs.existsSync(SESSION_FILE)) {
        ...
        if (sess && sess.cookieHeader) {
          const key = (sess.uid || sess.email || sess.cookieHeader.slice(0, 64));
          const existing = this.accounts.find(a => (a.uid || a.email || a.cookieHeader || '').slice(0, 64) === key);
          if (existing) {
    ...
    // Dedupe: ensure distinct accounts only
    const seen = new Set();
    const out = [];
  ...
  save() {
    try {
      ...
      fs.writeFileSync(POOL_FILE, JSON.stringify(clean, null, 2));
    } catch {}
  }
