...
const MIN_POOL = parseInt(process.env.ACCOUNT_POOL_MIN || '20', 10);
const MAX_POOL = parseInt(process.env.ACCOUNT_POOL_MAX || '20', 10);
      ...
      for (let attempt = 1; attempt <= HARVEST_RETRIES && !slotDone; attempt++) {
        try {
          ...
          if (session) {
            ...
            if (id) { ok++; slotDone = true; this.lastHarvestAt = Date.now(); this.lastHarvestError = ''; }
            else {
