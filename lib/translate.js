...
const MODE_TOKENS = [
  { token: '', label: '默认', search: true },
  { token: 'search', label: '搜索', search: true },
];

// RP 专属模型(酒馆/SillyTavern 角色扮演用):同一上游模型,但服务端会注入
// 沉浸式角色扮演提示词(含轻度破线破甲授权),与角色卡联动。
const RP_MODELS = [
  { id: 'sakana-namazu-rp', api: 'sakana-namazu', name: 'Namazu', tone: 'default', search: true },
  { id: 'sakana-fugu-rp', api: 'fugu', name: 'Fugu', tone: 'default', search: true },
  { id: 'sakana-namazu-polite-rp', api: 'sakana-namazu', name: 'Namazu', tone: 'jp-vibes', search: true },
  { id: 'sakana-namazu-osaka-rp', api: 'sakana-namazu', name: 'Namazu', tone: 'osaka', search: true },
];
...
function openaiRequestToSakana(body) {
  ...
  // sends both — web search always sends enableThinking:false). The user wants
  // thinking innate (always on), so when both are requested, thinking wins and
  // search is silently degraded (the model still gets the search-enhanced prompt
  // but the upstream flag is off).
  if (webSearchEnabled && enableThinking) webSearchEnabled = false;
