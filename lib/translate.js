...
function openaiRequestToSakana(body) {
  ...
  for (let i = msgs.length - 1; i >= 0; i--) {
    ...
    if (role === 'user') {
      ...
      if (typeof c === 'string') text = c;
        ...
        for (const part of c) {
          if (!part) continue;
          if (part.type === 'text' || part.type === 'input_text' || part.type === 'output_text') text += (part.text || part.content || '');
      ...
      // External framework executed a tool and returns the result: pass it to
      // the model as turn input (Sakana has no native tool engine for custom tools).
      const c = m.content;
      let txt = '';
  ...
  if (toolResults.length) {
    ...
    prompt = (prompt ? prompt + '\n\n' : '') + '以下是上一步调用的工具返回结果,请基于这些结果继续:\n' + parts;
  }

  // Inject a light tool hint so the model knows it MAY call the declared
  // custom tools (external frameworks intercept the text and run them).
  if (body.tools && !isContinue) {
    const custom = body.tools.filter((t) => t && t.type === 'function' && t.function && t.function.name && t.function.name !== 'search');
    if (custom.length && process.env.TOOL_PROMPT !== '0') {
      const hint = custom.map((t) => `- ${t.function.name}: ${t.function.description || '可用工具'}`).join('\n');
      prompt = (prompt ? prompt + '\n\n' : '') + `可用工具(如需要请以 JSON 形式调用,格式: {"tool":"名称","arguments":{...}}):\n${hint}`;
    }
  }
    ...
    switch (obj.type) {
      ...
      case 'finalAnswer': {
        ...
        if (text) {
          ...
          const prev = this._cleanPrev || '';
          // Incremental delta. long answers: V8 `startsWith` is memcmp-fast, so
          // re-scanning the whole accumulated text per event is cheap; only fall
          // back to a full scan when the prefix actually changed (rare).
          let i;
          if (cleaned.startsWith(prev)) i = prev.length;
          else {
            i = 0;
