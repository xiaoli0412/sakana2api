...
async function harvestSession() {
  ...
  log(`session saved: loggedIn=${session.loggedIn} cookies=${cookies.length} uid=${session.uid || '-'} email=${session.email || '-'}`);
  return session;
}
...
async function start() {
  ...
  timer = setInterval(() => { refreshSession().catch(() => {}); }, REFRESH_MS);
  return session;
}
...
module.exports = { autoSession: { start, stop, getSession, harvestSession, refreshSession } };